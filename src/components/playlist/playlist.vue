<template>
  <transition name="list-fade">
    <div class="playlist" v-show="showFlag" @click="closePlaylist">
        ...
        <scroll  class="list-content" ref="listContent" :data="sequenceList">
          <transition-group name="list" tag="ul">
            <li :key="item.id" class="item" ref="listItem" v-for="(item,index) in sequenceList" @click="selectItem(item,index)">
              <i class="current" :class="getcurrentCla(item)"></i>
              <span class="text">{{item.name}}</span>
              <span  class="like" @click.stop="triggerLove(item)">
      ...
      <confirm ref="confirmTips" text="是否清空播放列表" @confirm="confrmClear"></confirm>
...
<script type="text/ecmascript-6">
  ...
  import {loveMixin} from '../../common/js/mixin'
